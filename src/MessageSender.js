  ...
  const handleUpload = (event) => {
    ...
    if (!image) {
      ...
      const uploadTask = storage
        .ref(`images/${image.name == null ? image : image.name}`)
        .put(image);
      uploadTask.on(
        "state_changed",
        (snapshot) => {
        ...
        <div style={modalStyle} className={classes.paper}>
          <form className="modal__form">
            <div className="modal__top">
              <h3>Create Post</h3>
              <div onClick={handleClose} className="top__icon">
            ...
            />
            <div className={`image__preview ${image && "show"}`}>
              {/* {e.target.files[0].name.split(".").pop()== "mp4"? <video width="320" height="240" controls autoplay src={}></video>} */}
              <img id="blah" alt="" />
              <progress
                value={progress}
                max="100"
              ...
              />
              <p className={`image__preview ${progress && "show"}`}>
                Uploaded {progress}%
              </p>
            </div>
          ...
          {/* <h2 className={`imageText ${image && "show"}`}></h2> */}
        </div>
      </Modal>
